var Slipshow = (function (exports) {
	...
	class SlipFigure extends HTMLImageElement {
	  ...
	  set figureStep(step) {
	    this.promise = this.promise.then(() => {
	      if (step > this.maxStep) this.internalStep = this.maxStep;else if (step < 0) this.internalStep = 0;else this.internalStep = step;
	      this.updateSRC();
	    });
	  }
	...
	customElements.define('slip-figure', SlipFigure, {
	  extends: "img"
	});
	...
	function IController (ng) {
	  ...
	  document.addEventListener("keypress", ev => {
	    if (change_speed_keys.includes(ev.key) && activated) {
	  ...
	  document.addEventListener("keydown", ev => {
	    ...
	    if (down_keys.includes(ev.key) && activated) {
	      engine.moveWindowRelative(0, speedMove / openWindowHeight, 0, 0, 0.1);
	    } // Bas


	    if (up_keys.includes(ev.key) && activated) {
	      engine.moveWindowRelative(0, -speedMove / openWindowHeight, 0, 0, 0.1);
	    } // Haut


	    if (left_keys.includes(ev.key) && activated) {
	      engine.moveWindowRelative(-speedMove / openWindowWidth, 0, 0, 0, 0.1);
	    } // Gauche


	    if (right_keys.includes(ev.key) && activated) {
	      engine.moveWindowRelative(speedMove / openWindowWidth, 0, 0, 0, 0.1);
	    } // Droite


	    if (rotate_keys.includes(ev.key) && activated) {
	      engine.moveWindowRelative(0, 0, 0, 1, 0.1);
	    } // Rotate 


	    if (unrotate_keys.includes(ev.key) && activated) {
	      engine.moveWindowRelative(0, 0, 0, -1, 0.1);
	    } // Unrotate


	    if (zoom_keys.includes(ev.key) && activated) {
	      engine.moveWindowRelative(0, 0, 0.01, 0, 0.1);
	    } // Zoom


	    if (unzoom_keys.includes(ev.key) && activated) {
	      engine.moveWindowRelative(0, 0, -0.01, 0, 0.1);
	    } // Unzoom
	...
	function Slip(name, fullName, actionL, ng, options) {
	  ...
	  this.getMaxNext = () => {
	    ...
	    ["mk-visible-at", "mk-hidden-at", "mk-emphasize-at", "mk-unemphasize-at", "emphasize-at", "chg-visib-at", "up-at", "down-at", "center-at", "static-at", "exec-at", "enter-at", "focus-at", "unfocus-at", "figure-next-at", "figure-previous-at"].forEach(attr => {
	  ...
	  this.unpause = pause => {
	    if (pause.hasAttribute("static-at-unpause")) {
	      if (pause.getAttribute("static-at-unpause") == "") this.makeStatic(pause);else pause.getAttribute("static-at-unpause").split(" ").map(strID => {
	        this.makeStatic("#" + strID);
	      });
	    }

	    if (pause.hasAttribute("unstatic-at-unpause")) {
	      if (pause.getAttribute("unstatic-at-unpause") == "") this.makeUnStatic(pause);else pause.getAttribute("unstatic-at-unpause").split(" ").map(strID => {
	    ...
	    if (pause.hasAttribute("center-at-unpause")) {
	      if (pause.getAttribute("center-at-unpause") == "") this.moveCenterTo(pause, 1);else this.moveCenterTo("#" + pause.getAttribute("center-at-unpause"), 1);
	    }

	    if (pause.hasAttribute("exec-at-unpause")) {
	      if (pause.getAttribute("exec-at-unpause") == "") this.executeScript(pause);else pause.getAttribute("exec-at-unpause").split(" ").map(strID => {
	        this.executeScript("#" + strID);
	      });
	    }

	    if (pause.hasAttribute("reveal-at-unpause")) {
	      if (pause.getAttribute("reveal-at-unpause") == "") this.reveal(pause);else pause.getAttribute("reveal-at-unpause").split(" ").map(strID => {
	        this.reveal("#" + strID);
	      });
	    }

	    if (pause.hasAttribute("hide-at-unpause")) {
	      if (pause.getAttribute("hide-at-unpause") == "") this.hide(pause);else pause.getAttribute("hide-at-unpause").split(" ").map(strID => {
	        this.hide("#" + strID);
	      });
	    }
	    ...
	    if (pause.hasAttribute("focus-at-unpause")) {
	      if (pause.getAttribute("focus-at-unpause") == "") this.focus(pause);else this.focus("#" + pause.getAttribute("focus-at-unpause"));
	    }

	    if (pause.hasAttribute("unfocus-at-unpause")) {
	      if (pause.getAttribute("unfocus-at-unpause") == "") this.unfocus(pause);else this.unfocus("#" + pause.getAttribute("unfocus-at-unpause"));
	    }
	  };
	  ...
	  this.doAttributes = () => {
	    ...
	    this.queryAll("*[figure-next-at]").forEach(elem => {
	      ...
	      if (toFigureNext.includes(actionIndex)) elem.figureStep++;
	    });
	  ...
	  this.reloadCanvas = () => {
	    ...
	    setTimeout(function () {
	      ...
	      q[1].replaceWith(canvas2); // if(element && element.firstChild && element.firstChild.firstChild)
	  ...
	  setTimeout(function () {
	    ...
	    canvas.classList.add("sketchpad", "drawing");
	    that.sketchpadCanvas = canvas;
	    if (element && element.firstChild && element.firstChild.firstChild) element.firstChild.firstChild.appendChild(canvas);else element.appendChild(canvas); // }, 0);
	    // canvas for highlighting 
	    // setTimeout(function() {
	    ...
	    canvas2.classList.add("sketchpad", "sketchpad-highlighting");
	    that.sketchpadCanvasHighlight = canvas2;
	    if (element && element.firstChild && element.firstChild.firstChild) element.firstChild.firstChild.appendChild(canvas2);else element.appendChild(canvas2);
	    setTimeout(function () {
	      that.reloadCanvas();
	    }, 0);
	  }, 0); // names
